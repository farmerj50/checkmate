...
export default function Signup() {
  ...
  const handleSubmit = async () => {
    ...
    if (!isDemoMode && password !== confirm) { toast.error('Passwords do not match'); return; }
    setLoading(true);
    try {
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center px-6 py-12">
      ...
      >
        ...
        <p className="text-white/40 text-sm mb-10">Join thousands finding real connections</p>

        {isDemoMode && (
          <div className="mb-8 bg-amber-500/10 border border-amber-500/25 rounded-xl px-4 py-3 text-amber-300/80 text-xs">
            <strong className="text-amber-300">Demo mode</strong> — any email &amp; password will work.
          </div>
        )}
        ...
        <div className="space-y-5">
          ...
          {!isDemoMode && (
            <div className="border-b border-white/15 focus-within:border-pink-400 transition-colors pb-2">
              <label className="block text-white/35 text-xs mb-1.5 uppercase tracking-wider">Confirm password</label>
              <input
                type="password"
                value={confirm}
        ...
        <motion.button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl bg-pink-600 hover:bg-pink-500 disabled:opacity-50 text-white font-semibold shadow-xl shadow-pink-600/30 transition-colors mt-8"
        >
          {loading
        ...
        <p className="text-center text-white/20 text-xs mt-5">
          ...
          <span className="text-white/40">Terms</span> and{' '}
